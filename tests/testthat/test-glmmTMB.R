...
test_that("nbinom1 glmmTMB succeeds",{
  m1 <- glmmTMB(y ~ Ed + (1|So), data = MASS::UScrime, family = nbinom1())
  m2 <- glmmTMB(y ~ Ed + I(M.F/1000) + (1|So), data = MASS::UScrime, family = nbinom1())
  testthat::expect_no_error(mtab::h_tab(m1, m2))
})

test_that("nbinom2 glmmTMB succeeds",{
  m1 <- glmmTMB(y ~ Ed + (1|So), data = MASS::UScrime, family = nbinom2())
  m2 <- glmmTMB(y ~ Ed + I(M.F/1000) + (1|So), data = MASS::UScrime, family = nbinom2())
  testthat::expect_no_error(mtab::h_tab(m1, m2))
})
